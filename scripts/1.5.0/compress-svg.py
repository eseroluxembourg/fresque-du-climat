...
svg_filename = svg_filename[:-4] + ".svg"
svg_filename_copy = svg_filename[:-4] + "copy.svg"

command = f'scour -i { svg_filename } -o { svg_filename } --enable-viewboxing --enable-id-stripping --enable-comment-stripping --shorten-ids --indent=none'
subprocess.call(command, shell=True)
os.remove(svg_filename)
os.rename(svg_filename_copy, svg_filename)